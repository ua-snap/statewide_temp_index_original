...
stdate = datetime.date(2019, 6, 01)
enddate = datetime.date.today() + datetime.timedelta(days=-1)

#enddate = datetime.date(2020, 1, 23)
...
with open(localPath+'StationsList.txt') as csv_station_name_file:
  ...
  for row1 in csv_reader1:
    ...
    station_URL = 'http://data.rcc-acis.org/StnData?sid='+row1[0]+'&sdate='+s1+'&edate='+s2+'&elems=1,2&output=csv'
    response = urllib2.urlopen(station_URL)
    html = response.read()
    ...
    for x in html_list:
      if(len(x)>10):
        ...
        xx =x.split(',')
        if (xx[1]!="M" and xx[2]!="M"):        #look for entries where a valid max and min exist
          today_max = xx[1]
          today_min = xx[2]
          today_ave = ((int(today_max)+int(today_min)) / float(2.0)) 
          if (row1[2]=='PAOT' and today_year==2019):  #adjustment for Kotzebue
            ...
            if (today_month>=6 and today_month<=8):
              today_ave = today_ave - 4 
          if (row1[2]=='PANC' and today_year>=2017 and today_year<=2018 ):  #adjustment for Anchorage
              today_ave = today_ave - 2.0 
          if (row1[2]=='PANC' and today_year==2016 and today_month==12 ):  #adjustment for Anchorage
              today_ave = today_ave - 2.0 
          if (row1[2]=='PANC' and today_year==2020 and today_month==1 ):  #adjustment for Anchorage
              today_ave = today_ave - 2.0 
          ...
          with open(localPath+'StationsNormals.txt') as csv_normal_file:
            ...
            for row2 in csv_reader2:
              if (row2[0]==row1[0] and int(row2[2])==int(today_month) and int(row2[3])==int(today_day)):
                norm_ave = float(row2[6])
                norm_sd = float(row2[9])
                depart = float(today_ave) - float(norm_ave)
                depart_sd = (float(today_ave) - float(norm_ave)) / float(norm_sd) 
                depart_sd_weighted = (float(today_ave) - float(norm_ave)) / float(norm_sd) * float(row1[3])
          datastring = datastring + x[0:10]+','+row1[0]+','+row1[1]+',Ave,'+str(today_ave)+',Norm,'+str(norm_ave)+',Depart SD,'+str(round(depart_sd,3))+',Weight,'+str(row1[3]) +'\n'
          if(x[0:10]==s3):  #look for the last date
            lastdatestring = lastdatestring + row1[0]+','+row1[1]+','+ str(round(depart_sd,3)) +'\n'
    station_count+=1   # increment the station count (e.g., Anchorage is first, Barrow is second, etc.)
print('\n\n\n\n\n'+datastring)
...
for bb in range(0,numdays):
  ...
  if (dailyCount>0): 
    ...
    print(dateStr+','+str(round(dailyAvg,3))+','+str(dailyCount)+','+str(prob))
    summaryString = summaryString + dateStr+','+str(round(dailyAvg,3))+','+str(dailyCount)+','+str(prob)+','+str(round(dailyAvgTemp,3)) +'\n'
...
MOS_response = urllib2.urlopen(MOS_URL)
MOS_html = MOS_response.read()
    ...
    if(MOShour<99 and checkline==4):
      ...
      with open(localPath+'StationsNormals.txt') as csv_normal_file:
        ...
        for row6 in csv_reader6:
          if (row6[10]==x5 and int(row6[2])==day1date.month and int(row6[3])==day1date.day):
            day1_norm_ave = float(row6[6])
            day1_norm_sd = float(row6[9])
            day1temp = day1temp + (float(day1forecast * theWeight))
            day1sum = day1sum + (((float(day1forecast) - float(day1_norm_ave)) / float(day1_norm_sd)) * theWeight)
          if (row6[10]==x5 and int(row6[2])==day2date.month and int(row6[3])==day2date.day):
            day2_norm_ave = float(row6[6])
            day2_norm_sd = float(row6[9])
            day2temp = day2temp + (float(day2forecast * theWeight))
            day2sum = day2sum + (((float(day2forecast) - float(day2_norm_ave)) / float(day2_norm_sd)) * theWeight)
          if (row6[10]==x5 and int(row6[2])==day3date.month and int(row6[3])==day3date.day):
            day3_norm_ave = float(row6[6])
            day3_norm_sd = float(row6[9])
            day3temp = day3temp + (float(day3forecast * theWeight))
            day3sum = day3sum + (((float(day3forecast) - float(day3_norm_ave)) / float(day3_norm_sd)) * theWeight)
          if (row6[10]==x5 and int(row6[2])==day4date.month and int(row6[3])==day4date.day):
            day4_norm_ave = float(row6[6])
            day4_norm_sd = float(row6[9])
            day4temp = day4temp + (float(day4forecast * theWeight))
            day4sum = day4sum + (((float(day4forecast) - float(day4_norm_ave)) / float(day4_norm_sd)) * theWeight)
          if (row6[10]==x5 and int(row6[2])==day5date.month and int(row6[3])==day5date.day):
            day5_norm_ave = float(row6[6])
            day5_norm_sd = float(row6[9])
            day5temp = day5temp + (float(day5forecast * theWeight))
            day5sum = day5sum + (((float(day5forecast) - float(day5_norm_ave)) / float(day5_norm_sd)) * theWeight)
          if (row6[10]==x5 and int(row6[2])==day6date.month and int(row6[3])==day6date.day):
            day6_norm_ave = float(row6[6])
            day6_norm_sd = float(row6[9])
            day6temp = day6temp + (float(day6forecast * theWeight))
            day6sum = day6sum + (((float(day6forecast) - float(day6_norm_ave)) / float(day6_norm_sd)) * theWeight)
          if (row6[10]==x5 and int(row6[2])==day7date.month and int(row6[3])==day7date.day):
            day7_norm_ave = float(row6[6])
            day7_norm_sd = float(row6[9])
            day7temp = day7temp + (float(day7forecast * theWeight))
            day7sum = day7sum + (((float(day7forecast) - float(day7_norm_ave)) / float(day7_norm_sd)) * theWeight)
...
if (ww1<0.5): prob1 = str(round(0-(20*(0.5-ww1)),3))
if (ww1>=0.5): prob1 = '+'+str(round(20*(ww1-0.5),3))
ww2 = scipy.stats.norm(0, 0.71074).cdf(day2sum)
if (ww2<0.5): prob2 = str(round(0-(20*(0.5-ww2)),3))
if (ww2>=0.5): prob2 = '+'+str(round(20*(ww2-0.5),3))
ww3 = scipy.stats.norm(0, 0.71074).cdf(day3sum)
if (ww3<0.5): prob3 = str(round(0-(20*(0.5-ww3)),3))
if (ww3>=0.5): prob3 = '+'+str(round(20*(ww3-0.5),3))
ww4 = scipy.stats.norm(0, 0.71074).cdf(day4sum)
if (ww4<0.5): prob4 = str(round(0-(20*(0.5-ww4)),3))
if (ww4>=0.5): prob4 = '+'+str(round(20*(ww4-0.5),3))
ww5 = scipy.stats.norm(0, 0.71074).cdf(day5sum)
if (ww5<0.5): prob5 = str(round(0-(20*(0.5-ww5)),3))
if (ww5>=0.5): prob5 = '+'+str(round(20*(ww5-0.5),3))
ww6= scipy.stats.norm(0, 0.71074).cdf(day6sum)
if (ww6<0.5): prob6 = str(round(0-(20*(0.5-ww6)),3))
if (ww6>=0.5): prob6 = '+'+str(round(20*(ww6-0.5),3))
ww7 = scipy.stats.norm(0, 0.71074).cdf(day7sum)
if (ww7<0.5): prob7 = str(round(0-(20*(0.5-ww7)),3))
if (ww7>=0.5): prob7 = '+'+str(round(20*(ww7-0.5),3))

mosplotstring = day1date.strftime('%b %d')+': '+str(prob1)+'\n\n'+day2date.strftime('%b %d')+': '+str(prob2)+'\n\n'+day3date.strftime('%b %d')+': '+str(prob3)+'\n\n'+day4date.strftime('%b %d')+': '+str(prob4)+'\n\n'+day5date.strftime('%b %d')+': '+str(prob5)+'\n\n'+day6date.strftime('%b %d')+': '+str(prob6)+'\n\n'+day7date.strftime('%b %d')+': '+str(prob7)
mostempstring = day1date.strftime('%b %d')+': '+str(day1temp)+'\n\n'+day2date.strftime('%b %d')+': '+str(day2temp)+'\n\n'+day3date.strftime('%b %d')+': '+str(day3temp)+'\n\n'+day4date.strftime('%b %d')+': '+str(day4temp)+'\n\n'+day5date.strftime('%b %d')+': '+str(day5temp)+'\n\n'+day6date.strftime('%b %d')+': '+str(day6temp)+'\n\n'+day7date.strftime('%b %d')+': '+str(day7temp)

if (MOShour==0): filestring = "00_UTC_MOS_"+str(MOSyear)+"_"+"%02d" % (MOSmonth,)+"_"+"%02d" % (MOSday,)+".txt"
if (MOShour==12): filestring = "12_UTC_MOS_"+str(MOSyear)+"_"+"%02d" % (MOSmonth,)+"_"+"%02d" % (MOSday,)+".txt"


newmosstring = "MOS Issue Date,Forecast Date,Index Value\n"
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day1date.strftime('%b %d, %Y')+';'+str(prob1)+'\n'
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day2date.strftime('%b %d, %Y')+';'+str(prob2)+'\n'
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day3date.strftime('%b %d, %Y')+';'+str(prob3)+'\n'
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day4date.strftime('%b %d, %Y')+';'+str(prob4)+'\n'
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day5date.strftime('%b %d, %Y')+';'+str(prob5)+'\n'
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day6date.strftime('%b %d, %Y')+';'+str(prob6)+'\n'
newmosstring = newmosstring+MOSdate.strftime('%b %d, %Y')+';'+day7date.strftime('%b %d, %Y')+';'+str(prob7)+'\n'
...
for zzz in range(0,len(subsetdates)):
  tempdate = datetime.date(int(subsetdates[zzz][0:4]), int(subsetdates[zzz][5:7]), int(subsetdates[zzz][8:10]))
...
plt.title('Alaska Statewide Temperature Index: '+str(subsetdates2[0])+', '+subsetdates[0][0:4]+', to '+str(subsetdates2[daystoplot-1])+', '+subsetdates[daystoplot-1][0:4], fontweight = 'bold',fontsize='xx-large', y=1.05, **afont)



plt.bar(y_pos, subsetpos, color='r', edgecolor = "none", width=0.75, alpha=0.8, align='center')
plt.bar(y_pos, subsetneg, color='b', edgecolor = "none", width=0.75, alpha=0.8, align='center')


plt.xticks(y_pos_range[::frequency],subsetdates2[::frequency],rotation=90)
plt.yticks(np.arange(-10.,10.1,1))
plt.ylabel("<- Below Normal     Index     Above Normal ->", weight = 'bold', fontsize='large', **afont)
...
plt.text(14.5,-15, 'Index based on 1981-2010 daily normals and standard deviations of 25 stations. A value', size=13, **afont)
plt.text(14.5,-16, 'of +1 means that the day is warmer than 10% of all Above Normal days. A value of +8 means', size=13, **afont)
plt.text(14.5,-17, 'the day is warmer than 80% of all Above Normal days; and so on. The opposite is true of', size=13, **afont)
plt.text(14.5,-18, 'negative numbers. Data source: NCEI & ACIS.', size=13, **afont)
plt.text(50.5,-18, '30-day average shown as black line.', weight = 'bold',size=13, **afont)
plt.text(94,-12, str(subsetdates2[daystoplot-1]), weight = 'bold',size=17, **afont)
plt.text(98.5,6, str(MOShour)+' UTC MOS\nForecast', horizontalalignment = 'center',weight = 'bold',size=16, **afont)
...
os.system("magick.exe convert "+localPath+"map.png -resize 185x143 D:\\CS490\\Index\\map2.png")
os.system("magick.exe convert "+localPath+"OutputPlot.png "+localPath+"map2.png -geometry +1052+625 -composite D:\\CS490\\Index\\result.png")
os.system("magick.exe convert "+localPath+"result.png -bordercolor Black -border 1x1 "+localPath+"result2.png")

os.system("magick.exe convert "+localPath+"result2.png -bordercolor White -border 10x10 "+localPath+"plot90days.png")
os.chdir(localPath)
...
if (MOShour==0): filestring2 = "00_UTC_MOS_"+str(MOSyear)+"_"+"%02d" % (MOSmonth,)+"_"+"%02d" % (MOSday,)+".png"
if (MOShour==12): filestring2 = "12_UTC_MOS_"+str(MOSyear)+"_"+"%02d" % (MOSmonth,)+"_"+"%02d" % (MOSday,)+".png"

os.system("magick.exe convert "+localPath+"result2.png -bordercolor White -border 10x10 "+localPath+"Maps\\"+filestring2)
...
with open("plot90days.png", "rb") as f:
    s3.upload_fileobj(f, "daily-alaska-climate-graphics", "plot90days.png", ExtraArgs={'ContentType': 'image/png'})
    s3.upload_fileobj(f, "daily-alaska-climate-graphics", "Index_"+str(MOSyear)+"_"+"%02d" % (MOSmonth,)+"_"+"%02d" % (MOSday,)+".png", ExtraArgs={'ContentType': 'image/png'})
